...
class ImageRef(BaseModel):
    ...
    caption: str | None = Field(None, description="Optional caption for the image")
...
class Exercise(BaseModel):
    ...
    instructions: list[str] = Field(
        ..., description="Step-by-step instructions for performing the exercise", min_length=2
    )
    common_mistakes: list[str] = Field(default_factory=list, description="Common errors to avoid")

    # Categorization
    body_regions: list[BodyRegion] = Field(
        ..., description="Primary body regions targeted", min_length=1
    )
    ...
    contraindications: list[str] = Field(
        default_factory=list, description="Conditions where this exercise should NOT be used"
    )
    ...
    default_hold: str | None = Field(None, description="Hold duration per rep, e.g., '5 seconds'")
    default_rest: str = Field(default="30 seconds", description="Rest between sets")
    ...
    video_url: str | None = Field(None, description="Optional video demonstration URL")

    # Translations
    translations: dict[str, ExerciseTranslation] = Field(
        default_factory=dict, description="Cached translations keyed by language code"
    )
